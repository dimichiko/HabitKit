...
const CheckoutPage = () => {
  ...
  const getSelectedApps = (plan, apps) => {
  ...
  const handleRedeemCode = async () => {
    ...
    try {
      ...
      if (res.data.type === 'admin') await updateUser({ plan: 'full', role: 'admin' });
      if (res.data.type === 'full') await updateUser({ plan: 'full' });
      setTimeout(() => {
        navigate('/');
      }, 2000);
    } catch (err) {
