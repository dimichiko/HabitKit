...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-white to-pink-100 flex flex-col">
      ...
      <main className={`pt-24 max-w-5xl mx-auto px-4 flex flex-col items-center ${isLogged && (user.plan === 'Free' || user.plan === 'Full') ? 'mt-16' : ''}`}>
        {/* HERO VISUAL */}
        ...
        <Suspense fallback={null}>
          <div id="apps-section">
            <AppsSection onAppClick={appPath => {
              if (!isLogged) {
                navigate('/register');
              } else {
                navigate(appPath);
