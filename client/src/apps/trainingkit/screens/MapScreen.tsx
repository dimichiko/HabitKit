...
const MapScreen = ({ onNavigate }: MapScreenProps) => {
  ...
  const lugares = Array.from(new Set(trainings.map(t => t.location?.name).filter(Boolean)));
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        <h2 className="text-2xl font-bold flex items-center gap-2"><span>📍</span> Mapa de Entrenamientos</h2>
                ...
                <Popup>
                  ...
                  <div className="text-xs">
                    <span className="flex items-center gap-2">
                      <span>📍</span>
                      <span className="text-sm text-gray-600">{t.location?.name}</span>
                    </span>
                  </div>
