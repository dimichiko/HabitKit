...
interface CollapsedFolders {
  [folderName: string]: boolean;
}

interface ConfirmDeleteFolder {
  folderId: string;
  folderName: string;
  habitsCount: number;
}
...
const HomePage = ({ showAddModal, setShowAddModal }: HomePageProps) => {
  ...
  const [checkinCounts, setCheckinCounts] = useState<CheckinCounts>({});
  ...
  const getFolderProgress = (folderName: string): FolderProgress => {
    if (folderName === 'Todos los hábitos') {
      const totalCompleted = Object.values(checkinCounts).reduce((sum: number, count: number) => sum + count, 0);
      const totalTargets = allHabits.reduce((sum: number, habit: Habit) => sum + habit.timesPerDay, 0);
      return { completed: totalCompleted, total: totalTargets };
    }
    
    const folderHabits = allHabits.filter(h => h.folder === folderName);
    const completed = folderHabits.reduce((sum: number, habit: Habit) => sum + (checkinCounts[habit._id] || 0), 0);
    const total = folderHabits.reduce((sum: number, habit: Habit) => sum + habit.timesPerDay, 0);
                ...
                {folders.map(folder => {
                  ...
                  return (
                    <FolderCard
                      ...
                      onAddHabit={(folderName: string) => {
                        setNewHabit({ ...newHabit, folder: folderName });
                        setShowAddModal(true);
                      }}
                      onRename={(folderId: string, folderName: string) => {
