...
const customTileClass = ({ date, view }, checkins, habits) => {
  if (view !== 'month') return '';
  const ymd = date.toISOString().slice(0, 10);
  const dayCheckins = checkins.filter(c => c.date.slice(0, 10) === ymd);
  
  if (dayCheckins.length === 0) return 'no-activity-day';
  
  // Calcular si se cumplieron todas las metas del día
  const totalGoal = habits.reduce((sum, habit) => sum + habit.timesPerDay, 0);
...
const CalendarPage = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      for (const h of habits) {
        ...
        allCheckins = allCheckins.concat(cks.map(c => ({ ...c, habitId: h._id, habitName: h.name })));
      }
  ...
  const getTileTooltip = ({ date, view }) => {
    if (view !== 'month') return null;
    const ymd = date.toISOString().slice(0, 10);
    const dayCheckins = checkins.filter(c => c.date.slice(0, 10) === ymd);
    ...
    const habitList = Object.values(grouped).map(h => `${h.name} (${h.count})`).join(', ');
    const totalGoal = habits.reduce((sum, habit) => sum + habit.timesPerDay, 0);
    const totalCompleted = dayCheckins.length;
  ...
  const handleDayClick = (date) => {
    ...
    const details = Object.entries(grouped).map(([habitId, data]) => {
      ...
      <div style={{width:'100%',maxWidth:1100,background:'#fff',borderRadius:24,boxShadow:'0 4px 32px #0002',padding:'24px',margin:'24px 0',position:'relative'}}>
        {loading ? (
          ...
          <Calendar
            ...
            tileClassName={args => customTileClass(args, checkins, habits)}
            tileTitle={getTileTooltip}
