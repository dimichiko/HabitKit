...
const DashboardScreen = ({ onNavigate }) => {
  ...
  const loadCheckinCounts = async (habitsList) => {
    try {
      const counts = {};
      for (const habit of habitsList) {
        const { data } = await apiClient.get(`/habits/${habit._id}/checkins`);
        const todayYMD = new Date().toISOString().slice(0, 10);
        const todayCount = data.filter(c => c.date.slice(0, 10) === todayYMD).length;
  ...
  const completedToday = Object.values(checkinCounts).reduce((sum, count) => sum + count, 0);
  const totalTargets = habits.reduce((sum, habit) => sum + habit.timesPerDay, 0);
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {habits.filter(h => h.folder).length} organizados
                </p>
              </div>
