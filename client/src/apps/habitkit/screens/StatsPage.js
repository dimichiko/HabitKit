...
const StatsPage = () => {
  ...
  const [dateRange, setDateRange] = useState('7'); // 7, 30, current
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        ...
        const metaDiaria = habits.reduce((sum, h) => sum + h.timesPerDay, 0);
        setChartData({
          labels: fechas,
          datasets: [{
            label: 'Hábitos completados',
            data: fechas.map(f => dias[f]),
            backgroundColor: fechas.map((f, i) => {
              const value = dias[f];
        ...
        // Días top
        const topDias = Object.entries(dias).sort((a,b) => b[1]-a[1]).slice(0,3);
        const promedioSemanal = Object.values(dias).reduce((sum, val) => sum + val, 0) / Math.max(Object.keys(dias).length, 1);
        setStats({
          rachaActual,
          rachaMax,
          topDias,
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      title: { display: true, text: 'Rendimiento Diario', font: { size: 18, weight: '600' }, padding: { top: 10, bottom: 20 } },
      tooltip: {
