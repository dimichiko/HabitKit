...
const AnalyticsPage = () => {
  ...
  const chartData = {
    labels: stats?.dailyStats.map(d => new Date(d.date).toLocaleDateString('es-ES', { weekday: 'short' })) || [],
    datasets: [
      {
        label: 'Calorías',
        data: stats?.dailyStats.map(d => d.totalCalories) || [],
  ...
  const renderStatCard = (title, value, unit, icon, colorClass) => (
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <div className="max-w-7xl mx-auto p-6" style={{paddingTop:'80px'}}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 mb-10 border border-gray-100 dark:border-gray-700 grid grid-cols-1 md:grid-cols-2 gap-8 animate-fade-in">
          {renderStatCard('Promedio diario', formatNumber(stats.avgDailyCalories), 'cal', '📈', 'text-orange-600', true)}
          {renderStatCard('Días cumplidos', `${stats.daysWithEntries} / ${stats.totalDays}`, '', '🎯', 'text-green-600', true)}
          {renderStatCard('Mejor día', formatNumber(stats.maxDayCalories), 'cal', '🏆', 'text-blue-600', true)}
          {renderStatCard('Total periodo', formatNumber(stats.totalCalories, 0), 'cal', '📊', 'text-purple-600', true)}
        </div>
...
function MacroBlock({ label, value, unit, icon, color }) {
