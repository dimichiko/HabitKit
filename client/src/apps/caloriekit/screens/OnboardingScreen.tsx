...
interface OnboardingScreenProps {
  onComplete: (profileData: UserData & CalculatedGoals) => void;
}
...
interface StepComponentProps {
  ...
  currentStep?: number;
  totalSteps?: number;
  calculatedGoals?: CalculatedGoals | null;
  isSaving?: boolean;
}
...
// Goal Step Component
const GoalStep = ({ userData, updateUserData, onNext }: Pick<StepComponentProps, 'userData' | 'updateUserData' | 'onNext'>) => {
  const goals: Goal[] = [
    {
      id: 'lose',
      title: 'Perder Peso',
...
const ActivityStep = ({ userData, updateUserData, onNext, onBack }: Pick<StepComponentProps, 'userData' | 'updateUserData' | 'onNext' | 'onBack'>) => {
  ...
  return (
    <div>
      ...
      <div className="space-y-4 mb-8">
        {activityLevels.map((level: ActivityLevel) => (
          <button
            key={level.id}
...
const WeightGoalsStep = ({ userData, updateUserData, onNext, onBack }: Pick<StepComponentProps, 'userData' | 'updateUserData' | 'onNext' | 'onBack'>) => {
  return (
    <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => updateUserData({ weeklyWeightGoal: parseFloat(e.target.value) || 0 })}
              className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent"
              placeholder="0.5"
              min="0"
              max="5"
