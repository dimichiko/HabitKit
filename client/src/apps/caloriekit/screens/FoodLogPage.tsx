...
const FoodLogPage = ({ userProfile }: FoodLogPageProps) => {
  const { 
    meals, 
    loading, 
    error, 
    ...
    deleteMeal, 
    setError 
  } = useMealContext() as any;
  
  const [showMealEntry, setShowMealEntry] = useState<boolean>(false);
  const [activeMealType, setActiveMealType] = useState<string>('Desayuno');

  const dailyGoal = userProfile?.calorieTarget || 2000;

  const handleMealAdd = async (mealData: any) => {
    try {
      await addMeal({
        ...mealData,
  ...
  const getMealsByType = (type: string): any[] => {
    const englishType = mapMealTypeToEnglish(type);
    return meals.filter((meal: any) => meal.mealType === englishType);
  };

  const getTotalCaloriesByType = (type: string): number => {
    return getMealsByType(type).reduce((sum: number, meal: any) => sum + (meal.calories || 0), 0);
  };
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-8 min-h-[350px] transition-all duration-500 border border-gray-100 dark:border-gray-700">
              ...
              {getMealsByType(activeMealType).length === 0 ? (
                ...
                <div className="space-y-4 animate-fade-in">
                  {getMealsByType(activeMealType).map((meal: any) => (
                    <div 
                      key={meal._id} 
