...
export const calculateCaloriesFromMacros = (protein: number, carbs: number, fat: number): number => {
  return (protein * 4) + (carbs * 4) + (fat * 9);
};

// Calculate macronutrient percentages
export const calculateMacroPercentages = (protein: number, carbs: number, fat: number): MacroPercentages => {
...
export const calculateIdealWeightRange = (height: number, gender: Gender): WeightRange => {
  const heightInMeters = height / 100;
  const bmiMin = 18.5;
  const bmiMax = 24.9;
...
export const calculateWaterNeeds = (weight: number, activityLevel: ActivityLevel = 'moderate'): number => {
...
export const calculateProteinNeeds = (weight: number, activityLevel: ActivityLevel = 'moderate'): number => {
...
export const calculateWeightLossDeficit = (currentWeight: number, targetWeight: number, timeframeWeeks: number): number => {
...
export const calculateWeightGainSurplus = (currentWeight: number, targetWeight: number, timeframeWeeks: number): number => {
...
export const calculateMealTiming = (dailyCalories: number, mealCount: number = 3): MealCalories => {
  const baseMealCalories = dailyCalories / mealCount;
  
  const mealDistribution: Record<number, MealDistribution> = {
  ...
  Object.keys(distribution).forEach((meal: string) => {
...
export const calculateNutritionScore = (meals: Meal[], dailyGoal: number): number => {
  if (meals.length === 0) return 0;
  
  const totalCalories = meals.reduce((sum: number, meal: Meal) => sum + meal.calories, 0);
  const totalProtein = meals.reduce((sum: number, meal: Meal) => sum + meal.protein, 0);
  const totalCarbs = meals.reduce((sum: number, meal: Meal) => sum + meal.carbs, 0);
  const totalFat = meals.reduce((sum: number, meal: Meal) => sum + meal.fat, 0);
...
export const getNutritionScoreFeedback = (score: number): NutritionFeedback => {
  if (score >= 90) {
    return { 
      level: 'Excelente', 
