...
const WeightTracker = () => {
  ...
  const [formData, setFormData] = useState<{ weight: string; notes: string }>({
    weight: '',
    notes: ''
  });
  ...
  const loadWeightData = async () => {
    ...
    try {
      ...
      if (errMsg.includes('Respuesta vacía') || errMsg.includes('404')) {
        setLatestWeight(null);
        setWeightStats({
          totalEntries: 0,
          averageWeight: 0,
  ...
  const getWeightTrend = (): Trend | null => {
  ...
  const getTrendIcon = (trend: Trend | null) => {
    if (!trend) return '⚖️';
    return trend.isGaining ? '📈' : '📉';
  };

  const getTrendColor = (trend: Trend | null) => {
    if (!trend) return 'text-gray-500';
    return trend.isGaining ? 'text-red-500' : 'text-green-500';
  };

  const getTrendText = (trend: Trend | null) => {
