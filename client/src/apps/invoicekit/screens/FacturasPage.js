...
const FacturasPage = ({ invoices, setInvoices, clientes, productos, currentCompany }) => {
  const [showForm, setShowForm] = useState(false);
  const [editingFactura, setEditingFactura] = useState(null);
  const [form, setForm] = useState({ clienteId: '', items: [], fechaVencimiento: '' });
  ...
  useEffect(() => {
    const subtotal = form.items.reduce((acc, it) => acc + it.cantidad * it.precio, 0);
    setForm(f => ({ ...f, subtotal, total: subtotal }));
  }, [form.items]);

  const handleShowForm = (factura = null) => {
    if (factura) {
      setEditingFactura(factura);
      setForm({ ...factura, clienteId: factura.clienteId || '' });
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const facturaData = {
        ...
        subtotal: form.items.reduce((sum, item) => sum + (item.cantidad * item.precio), 0),
        total: form.items.reduce((sum, item) => sum + (item.cantidad * item.precio), 0),
  ...
  const handleDelete = async (facturaId) => {
    ...
    try {
      ...
      setInvoices(invoices.filter(inv => inv._id !== facturaId));
    } catch (error) {
      console.error("Error eliminando factura:", error);
    }
  };

  const handleTogglePayment = async (facturaId) => {
    try {
        const updatedFactura = await toggleFacturaPayment(facturaId);
        setInvoices(invoices.map(inv => inv._id === facturaId ? updatedFactura : inv));
    } catch (error) {
  ...
  // Métrica mensual
  const now = new Date();
  const monthInvoices = invoices.filter(inv => new Date(inv.fechaEmision).getMonth() === now.getMonth() && new Date(inv.fechaEmision).getFullYear() === now.getFullYear());
  ...
  return (
    <div className="container mx-auto px-4 py-8" style={{paddingTop:'80px'}}>
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-2 mb-6">
        ...
        <input type="text" placeholder="Buscar por cliente o N°" value={search} onChange={e=>setSearch(e.target.value)} className="border rounded px-3 py-1 text-sm w-full md:w-64" />
      </div>
      {/* Botón flotante + */}
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 bg-white p-4 rounded-xl shadow-md fixed top-1/2 left-1/2 z-50 transform -translate-x-1/2 -translate-y-1/2 w-full max-w-lg">
          <div className="mb-2">
            ...
            <select id="clienteId" value={form.clienteId} onChange={e=>setForm(f=>({...f, clienteId:e.target.value}))} className="border rounded p-2 ml-2">
              <option value="">Selecciona</option>
              {clientes.map((c,i)=>(<option key={i} value={c._id}>{c.nombre}</option>))}
            </select>
          </div>
          <div className="mb-2">
            <label htmlFor="fechaVencimiento">Fecha de vencimiento:</label>
            <input type="date" id="fechaVencimiento" value={form.fechaVencimiento} onChange={e=>setForm(f=>({...f, fechaVencimiento:e.target.value}))} className="border rounded p-2 ml-2" />
          </div>
          <div className="mb-2">
            <label htmlFor="productos">Productos:</label>
            <div className="flex gap-2 mt-2">
              <select id="productos" value={item.nombre} onChange={e=>{
                ...
                {productos.map((p)=>(<option key={p._id} value={p.nombre}>{p.nombre} - ${p.precio}</option>))}
              </select>
              <input type="number" placeholder="Cant." value={item.cantidad} onChange={e=>setItem({...item, cantidad: parseInt(e.target.value) || 0})} className="border rounded p-2 w-20" />
              <input type="number" placeholder="Precio" value={item.precio} onChange={e=>setItem({...item, precio: parseFloat(e.target.value) || 0})} className="border rounded p-2 w-24" />
              ...
              {form.items.map((it, i) => (
                <div key={it.id || i} className="flex justify-between items-center p-2 bg-gray-50 rounded mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button type="button" onClick={()=>setForm(f=>({...f, items: f.items.filter((_, index) => index !== i)}))} className="text-red-500">×</button>
                  </div>
                </div>
              ))}
            </div>
