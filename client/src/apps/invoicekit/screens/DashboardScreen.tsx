...
interface TopClient extends ClientTotal {
  clientId: string;
}
...
const DashboardScreen = ({ invoices, clientes, onNavigate }: DashboardScreenProps) => {
  ...
  const paidInvoices = invoices.filter((inv: Invoice) => inv.pagada).length;
  const pendingInvoices = totalInvoices - paidInvoices;
  const totalRevenue = invoices.filter((inv: Invoice) => inv.pagada).reduce((sum: number, inv: Invoice) => sum + inv.total, 0);
  const pendingRevenue = invoices.filter((inv: Invoice) => !inv.pagada).reduce((sum: number, inv: Invoice) => sum + inv.total, 0);
  ...
  const getCurrencySymbol = (code: string): string => {
    const currencies: Record<string, string> = { USD: '$', EUR: '€', GBP: '£', MXN: '$', COP: '$', ARS: '$' };
  ...
  const getRecentInvoices = (): Invoice[] => {
    return invoices
      .sort((a: Invoice, b: Invoice) => new Date(b.fecha).getTime() - new Date(a.fecha).getTime())
      .slice(0, 5);
  };

  const getTopClients = (): TopClient[] => {
    const clientTotals: Record<string, ClientTotal> = {};
    invoices.forEach((invoice: Invoice) => {
      ...
      clientTotals[invoice.clienteId].pagos.push(invoice.pagada ? (new Date(invoice.fechaPago || '').getTime() - new Date(invoice.fecha).getTime()) : null);
      if (!clientTotals[invoice.clienteId].last || new Date(invoice.fecha).getTime() > new Date(clientTotals[invoice.clienteId].last!.fecha).getTime()) {
  ...
  const unsentInvoices = invoices.filter((inv: Invoice) => !inv.enviada).length;

  // Widget resumen mensual ejemplo
  const currentMonth = currentTime.getMonth();
  const currentYear = currentTime.getFullYear();
  const monthInvoices = invoices.filter((inv: Invoice) => new Date(inv.fecha).getMonth() === currentMonth && new Date(inv.fecha).getFullYear() === currentYear);
  const monthTotal = monthInvoices.reduce((sum: number, inv: Invoice) => sum + inv.total, 0);
  const monthPaid = monthInvoices.filter((inv: Invoice) => inv.pagada).length;
          ...
          {invoices.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {getRecentInvoices().map((invoice: Invoice, index: number) => (
        ...
        <div className="bg-white rounded-xl shadow-sm p-6">
          ...
          {getTopClients().length === 0 ? (
            ...
            <div className="grid gap-4">
              {getTopClients().map((client: TopClient, index: number) => (
                <div key={`client-${client.clientId}-${index}`} className="flex flex-col md:flex-row items-center justify-between p-4 bg-gray-50 rounded-lg gap-2">
                  ...
                  <div className="flex flex-col items-end gap-1">
                    <span className="text-xs text-gray-500">Promedio pago: {(() => {
                      const validPagos = client.pagos.filter((pago): pago is number => pago !== null);
                      return validPagos.length > 0 ? (validPagos.reduce((a: number, b: number) => a + b, 0) / validPagos.length / 86400000).toFixed(1) + ' días' : 'N/A';
                    })()}</span>
